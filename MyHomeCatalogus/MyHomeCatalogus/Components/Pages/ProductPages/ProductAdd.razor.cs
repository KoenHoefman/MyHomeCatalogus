	...
	public partial class ProductAdd
	{
		...
		private async Task AddProduct()
		{
			...
			try
			{
				...
				var addedEntity = await ProductService.Add(Product);

				ToastService.ShowToast($"Product '{addedEntity.Name}' was successfully added.", ToastLevel.Success);

				NavigationManager.NavigateTo(RouteConstants.GetDetailRoute(RouteConstants.ProductBaseRoute, addedEntity.Id));
			}
			...
			catch (Exception ex)
			{
				...
				ToastService.ShowToast($"Error adding product: {ex.Message}", ToastLevel.Error, true);
			}
		}
